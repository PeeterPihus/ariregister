...
if (isset($_POST['signup'])) {
    ...
    if ($count == 0) { // if email is not found add user
        ...
        $stmts = $conn->prepare("INSERT INTO users(userName,email,pass,phone,name,surname) VALUES(?, ?, ?, ?, ?, ?)");
        $stmts->bind_param("ssssss", $uname, $email, $password, $phone, $name, $surname);
        $res = $stmts->execute();//get result
        $stmts->close();

        $user_id = mysqli_insert_id($conn);
        if ($user_id > 0) {
            $_SESSION['userName'] = $user_id; // set session and redirect to index page
            if (isset($_SESSION['userName'])) {
                print_r($_SESSION);
                header("Location: index.php");
                exit;
